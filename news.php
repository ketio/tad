<!DOCTYPE html>
<html>
	<head>
	<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
	<title>大內藝術節｜媒體報導</title>
	<link type="text/css" rel="stylesheet" href="css/common.css"/>
	</head>
	
	<body>
		<div id="content_wrapper">

		<!-- header -->
		<?php 
			include_once "header.php"; 
		?>

		<div id="content_news">
					<div class="list_news">
						<div>
							<div class="date_news">
								2014／11／14
							</div>
							<div class="press_news">
								自由時報：〈台北都會〉大內藝術街區 年底前推24檔展覽
							</div>
						</div>
						<div class="content_news">
								台北創意街區又多一個！文化局串聯內湖大直二十多家畫廊與藝術機構，打造北市第一個國際藝術街區，並以「城市精神」為主題，推出第一屆「台北大內藝術節」，即日起在捷運劍南路站有「城市精神攝影展」⋯⋯
						</div>
						<div class="more_news">
								<a href="http://news.ltn.com.tw/news/local/paper/830133" target="_BLANK"> more 〉</a>
						</div>
					</div>
					<div class="list_news">
						<div>
							<div class="date_news">
								2014／11／14
							</div>
							<div class="press_news">
								聯合報：大內藝術節…美麗華前廣場 變身創意街區
							</div>
						</div>
						<div class="content_news">
								「2014台北大內藝術節」昨天開展，美麗華百貨前「中山21號廣場」十數個大型攝影作品，與街景融為一體，成為創意街頭美術館。首屆台北大內藝術節以創意街區方式，讓經過的民眾能夠欣賞5位國內外藝術家⋯⋯
						</div>
						<div class="more_news">
								<a href="http://udn.com/NEWS/DOMESTIC/DOM2/9064915.shtml" target="_BLANK"> more 〉</a>
						</div>
					</div>
					<div class="list_news">
						<div>
							<div class="date_news">
								2014／11／13
							</div>
							<div class="press_news">
								自立晚報：串連21家畫廊及藝術機構 大內藝術節內湖展出
							</div>
						</div>
						<div class="content_news">
								臺北創意街區地圖上的新亮點：大內藝術區，為大直與內湖創造新的有機人文生態，並且大聲宣示既有專業價值創造、又有人文藝術創意無限的臺北城市精神！正式成為臺北第13個創意街區大內藝術區⋯⋯
						</div>
						<div class="more_news">
								<a href="https://tw.news.yahoo.com/%E4%B8%B2%E9%80%A321%E5%AE%B6%E7%95%AB%E5%BB%8A%E5%8F%8A%E8%97%9D%E8%A1%93%E6%A9%9F%E6%A7%8B-%E5%A4%A7%E5%85%A7%E8%97%9D%E8%A1%93%E7%AF%80%E5%85%A7%E6%B9%96%E5%B1%95%E5%87%BA-124753850.html" target="_BLANK"> more 〉</a>
						</div>
					</div>
					<div class="list_news">
						<div>
							<div class="date_news">
								2014／10／24
							</div>
							<div class="press_news">
								藝術網：【展覽】《人形雕》日本木雕藝術家聯展
							</div>
						</div>
						<div class="content_news">
								為配合臺北大內藝術區 (TAD) 成立之聯合開幕，青雲畫廊正式推出日本木雕藝術家聯展，由策展人–賴永興策展，「人‧形‧雕」日本木雕藝術家聯展是由前田忠一和吉田敦兩位日籍藝術家和台籍藝術家–賴永興⋯⋯
						</div>
						<div class="more_news">
								<a href="http://www.arttime.com.tw/ArtNews/news_group_cont1.aspx?AGDID=9826" target="_BLANK"> more 〉</a>
						</div>
					</div>
					<div class="list_news">
						<div>
							<div class="date_news">
								2014／10／06
							</div>
							<div class="press_news">
								典藏投資：「親愛的... 我想對你說」重新檢視與藝術的親密關係
							</div>
						</div>
							<div class="content_news">
								邁入第 21 個年頭，臺北國際藝術博覽會進入一個嶄新的時代，「Art Taipei 2014」將與藏家一起探索藝術的本質，分享對藝術的愛、恨、情、愁，和藝術一起談場戀愛。臺北藝博藝術總監林怡華笑著說⋯⋯
							</div>
							<div class="more_news">
								<a href="http://artouch.com/m_test/investments/story.aspx?aid=2014100618033" target="_BLANK"> more 〉</a>
							</div>	
					</div>

					<div class="list_news">
						<div>
							<div class="date_news">
								2014／07／31
							</div>
							<div class="press_news">
								中國時報：打造大內藝術區 讓畫廊成街景
							</div>
						</div>
						<div class="content_news">
								「打出「藝術造街」構想，中華民國畫廊協會推動「台北大內藝術區」，昨（30 日）宣告成立，匯集台北市大直、內湖地區的畫廊與藝術空間，目前共有 18 個單位、21 個空間坐落在這片新興區塊，未來將共推展覽，並於每年台北國際⋯⋯
						</div>
						<div class="more_news">
								<a href="http://www.chinatimes.com/newspapers/20140731000897-260115" target="_BLANK"> more 〉</a>
						</div>
					</div>

					<div class="list_news">
						<div>
							<div class="date_news">
								2014／07／31
							</div>
							<div class="press_news">
								中國時報：台北大內藝術區成立 畫廊密度居全台之冠
							</div>
						</div>
							<div class="content_news">
								看好大直與內湖地區廠辦空間大、租金相對便宜，與當地科技業、豪宅戶購藏藝術品的潛力，已有累積超過 20 家的畫廊和藝文空間進駐，全台密度最高，昨（30）日這些業者進一步宣布成立「台北大內藝術區」⋯⋯
							</div>
							<div class="more_news">
								<a href="http://www.chinatimes.com/newspapers/20140731000135-260204" target="_BLANK"> more 〉</a>
							</div>	
					</div>
					<div class="list_news">
						<div>
							<div class="date_news">
								2014／7／30
							</div>
							<div class="press_news">
								新網：大內藝術區成立 效仿北京798產生群聚
							</div>
						</div>
						<div class="content_news">
								「台北大內藝術區聯誼會」集結20餘家畫廊和藝文空間，今日在台北市大直和內湖地區成立。首屆會長李敦朗期許能像中國北京798藝術區、美國紐約雀兒喜區一樣，產生群聚效果⋯⋯
						</div>
						<div class="more_news">
								<a href="http://newnet.tw/Newsletter/Comment.aspx?Iinfo=5&iNumber=15132#ixzz3J1S2zLEt" target="_BLANK"> more 〉</a>
						</div>
					</div>
					<div class="list_news">
						<div>
							<div class="date_news">
								2014／07／30
							</div>
							<div class="press_news">
								非池中藝術網：台北大內藝術區成立
							</div>
						</div>
							<div class="content_news">
								「藝術」－是台北市重要的城市風景
								<br>
								來過台灣的遊客，都感同身受：台灣最美的風景是「人」。而遊歷台北市之後，無不讚嘆台北都會散發的人文氣質、整潔有序、以及美食的多樣性。
							</div>
							<div class="more_news">
								<a href="http://artemperor.tw/tidbits_page.php?table=news&id=5318?1" target="_BLANK"> more 〉</a>
							</div>
					</div>

					<div class="list_news">
						<div>
							<div class="date_news">
								2014／07／30
							</div>
							<div class="press_news">
								今日報：城市中最優美的風景 台北大內藝術區成立
							</div>
						</div>
							<div class="content_news">
								為了推動「台北也有一個國際矚目的畫廊區」－社團法人中華民國畫廊協會（以下簡稱畫廊協會）積極運作聯結台北市大直和內湖地區的畫廊和藝文空間⋯⋯
							</div>
							<div class="more_news">
								<a href="http://www.todaynews.com.tw/bencandy.php?fid-6-id-7125-page-1.htm" target="_BLANK"> more 〉</a>
							</div>
					</div>

					<div class="list_news">
						<div>
							<div class="date_news">
								2014／07／30
							</div>
							<div class="press_news">
								非池中藝術網：大內藝術區成立 畫廊做鄰居
							</div>
						</div>
							<div class="content_news">
								美國紐約雀兒喜區將工業荒地轉換為藝術聚落，群聚了百餘家畫廊；中國北京798藝術區則從聯合工廠吸引藝術家進駐開始，現在為北京畫廊重鎮⋯⋯
							</div>
							<div class="more_news">
								<a href="http://artemperor.tw/news_page.php?table=focus&id=506?1" target="_BLANK"> more 〉</a>
							</div>
					</div>
					
					<div class="list_news">
						<div>
							<div class="date_news">
								2014／07／16
							</div>
							<div class="press_news">
								街廓再造，藝術的經濟力
							</div>
						</div>
							<div class="content_news">
								藝術村或藝術聚落存在的意義為何？如「空場」提供藝術家一個友善的創作環境，大家可以彼此交流，加深對於自己創作的看法，並且舉辦活動幫助藝術家打開視野，而藝術聚落最主要的⋯⋯
							</div>
							<div class="more_news">
								<a href="http://www.artouch.com/artco/story.aspx?aid=2014071616641" target="_BLANK"> more 〉</a>
							</div>
					</div>

					<div class="list_news">
						<div>
							<div class="date_news">
								2014／07／16
							</div>
							<div class="press_news">
								名為藝術街廓的現代都會
							</div>
						</div>
							<div class="content_news">
								從文化的歷史角度，藝術街廓（arts districts）可說是在後工業化時期興起的一種空間觀念，反映著文化群體運用空間資源時的各種詮釋，以及人類在改變地表的行動上所累積的結果。⋯⋯
							</div>
							<div class="more_news">
								<a href="http://www.artouch.com/artco/story.aspx?aid=2014071616068" target="_BLANK"> more 〉</a>
							</div>
					</div>
					
		</div>
		
		<div id="footer"></div>
	</body>
</html>